...
- (void) main {
	...
	if ( ([self.agencyShortTitle  isEqualToString: @"actransit"])||([self.agencyShortTitle  isEqualToString: @"sf-muni"]) ) fetchedPredictions = [self fetchNextBusPredictions];
...
- (NSDictionary *) fetchNextBusPredictions {
	...
	if ([self.agencyShortTitle isEqualToString: @"sf-muni"]) urlString = [NSString stringWithFormat:@"http://webservices.nextbus.com/service/publicXMLFeed?command=predictionsForMultiStops&a=sf-muni&t=0"];
